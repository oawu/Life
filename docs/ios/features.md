...
6. initAfterLogin 流程：
   ...
   c. Response：帳本 metadata（不含 expenses）+ uploadedExpenses
   d. rebuildFromState → mergeExpenses（上傳的開銷）
   e. 清除 GuestExpense
   f. refreshViaManifest()（差異拉取其餘開銷）
...
| POST | /api/auth/init | expenses[] | InitResponse | 登入初始化（不含 expenses） |
| GET | /api/state | — | StateResponse | 取得完整 state（fallback） |
| GET | /api/manifest | — | ManifestResponse | Manifest 差異同步 |
| POST | /api/ledgers/:id/expenses/fetch | ids[] | ExpenseFetchResponse | 批次取得開銷 |
...
Watch App 需要 iPhone 登入後才能使用。token 透過 WatchConnectivity 從 iPhone 推送到 Watch 並持久化至 UserDefaults。

- 無 token → 顯示 `WatchLoginRequiredView`（「請在 iPhone 登入」）
- 有 token → 顯示記帳 Wizard
- Token 過期（API 401）→ 清除 token → 自動切回登入提示
- iPhone 登出 → 推送 `isLoggedIn: false` → Watch 清除 token

### Watch 獨立 API

Watch 透過 `WatchAPIClient` 直接呼叫後端 API，不依賴 iPhone 轉發：

| 動作 | API | 說明 |
|------|-----|------|
| 取得帳本 | `GET /api/state` | App 啟動時 + 收到新 token 時觸發 |
| 建立開銷 | `POST /api/ledgers/:id/expenses` | 直接建立，失敗存入離線佇列 |

- **離線佇列**：`OfflineExpense` 存入 UserDefaults（上限 50 筆），下次啟動或儲存成功後自動重送
- **帳本快取**：`CachedWatchLedger` 系列 struct 序列化至 UserDefaults，API 失敗時作為 fallback

### WatchConnectivity（iPhone → Watch 單向推送）

| 方法 | 內容 |
|------|------|
| `updateApplicationContext` | token、isLoggedIn、isOnline、帳本列表（持久化） |
| `sendMessage` | 同上（即時推送，Watch 可達時） |

- `PhoneSessionManager` 在帳本變更時推送 applicationContext + sendMessage
- `isAuthResolved` 門控：iPhone 啟動時 auth 尚未確認（`.launching`），`syncLedgersToWatch()` 被擋住，避免送出錯誤的 `isLoggedIn: false`。auth 從 `.launching` 轉換後才解鎖
- `WatchSessionManager` 接收後更新 `WatchExpenseStore`（@MainActor 隔離）；Watch 端 token 清除僅依據 iPhone 確認的 `isLoggedIn == false` 或 API 401
- 離線提示：帳本列表底部顯示「待上傳 N 筆」+「離線中」

---

## 11. 同步機制（Manifest Diff Sync）

### 架構

採用 manifest-based 差異同步，避免每次全量拉取所有開銷。Server 回傳輕量 manifest（開銷 `"id-version|..."` 清單 + 帳本 metadata），iOS 比對本地快取後只拉取差異。

```
正常路徑：refreshState()
├─ 1. syncOfflineExpenses()     ← 先上傳離線開銷
├─ 2. refreshViaManifest()      ← 差異同步（主路徑）
└─ 3. catch → refreshViaFullState()  ← fallback 全量

登入路徑：initAfterLogin()
├─ 1. POST /api/auth/init       ← 上傳 guest 開銷，回傳 metadata + uploadedExpenses
├─ 2. rebuildFromState + mergeExpenses
├─ 3. 清除 GuestExpense
└─ 4. refreshViaManifest()      ← 差異拉取其餘開銷
```

### refreshViaManifest 流程

```
1. GET /api/manifest → ManifestResponse
2. 偵測已刪除帳本（本地有、Server 沒有）→ 刪除本地快取
3. 偵測新帳本（Server 有、本地沒有）→ createLedgerFromManifest
4. 逐帳本比對 version → 不同則 rebuildLedgerMetadata
5. 逐帳本 diffWithManifest → 收集 needFetchIds + needDeleteLocalIds
6. 批次刪除本地已移除的開銷
7. totalFetchCount > 50 → 顯示 SyncProgressOverlay
8. 分批 POST /api/ledgers/:id/expenses/fetch（每批 200）
9. mergeExpenses（增量合併：有 serverId 就更新，沒有就新增）
10. 更新 syncProgress → 完成後 reload()
```

### 離線同步流程

```
1. 取得所有 isSynced = false 的開銷
2. 依帳本分組
3. 逐帳本 POST /api/ledgers/:id/expenses/batch
4. 失敗時指數退避重試（1s → 2s，最多 3 次）
5. 成功 → markExpensesSynced(localId, serverId, version)
```

### diff 邏輯

```
parseManifest("100-1|101-2|102-1")
  → [(id: 100, version: 1), (id: 101, version: 2), (id: 102, version: 1)]

diffWithManifest(manifest):
  遍歷 manifest：
    ├─ Server 有、本地沒有 → needFetch
    ├─ Server 有、本地 version 不同 → needFetch
    └─ 本地有（isSynced）、Server 沒有 → needDelete
```

### 版本遞增時機

| 操作 | Ledger.version | Expense.version |
|------|:-:|:-:|
| 帳本名稱/幣別變更 | +1 | — |
| 成員加入/退出 | +1 | — |
| 分類 CRUD / 排序 | +1 | — |
| 分類刪除（cascade） | +1 | 受影響 +1 |
| 固定開銷 CRUD | +1 | — |
| 結算（settle） | +1 | 受影響 +1 |
| 開銷建立 | — | 初始 1 |
| 開銷更新 | — | +1 |
...
`SyncProgressOverlay`：當需要 fetch 的開銷數量 > 50 筆時顯示。

- 半透明背景 + `.ultraThinMaterial` 圓角卡片
- `"正在同步開銷資料"` 標題
- `ProgressView(value:total:)` 進度條
- `"completed / total"` 筆數文字

### 已串接的後端 API

| Method | Path | Body | Response | 說明 |
|--------|------|------|----------|------|
| GET | /api/manifest | — | ManifestResponse | 取得 manifest |
| POST | /api/ledgers/:id/expenses/fetch | ids[] (max 200) | ExpenseFetchResponse | 批次取得開銷 |
| GET | /api/state | — | StateResponse | 全量 fallback |
| POST | /api/ledgers/:id/expenses/batch | expenses[] | ExpenseBatchResponse | 離線同步上傳 |
