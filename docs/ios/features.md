...
| 方向 | 方法 | 內容 |
|------|------|------|
| iPhone → Watch | `updateApplicationContext` + `sendMessage` | 帳本列表、分類、成員、幣別、isLoggedIn、isOnline |
| Watch → iPhone | `sendMessage` / `transferUserInfo` | 新增的開銷資料 |

- iPhone 端：`PhoneSessionManager` 收到 Watch 開銷後呼叫 API 建立 + 更新快取
- iPhone → Watch 雙通道：`updateApplicationContext`（持久化）+ `sendMessage`（即時推送，Watch 可達時）
- Watch 端：`WatchSessionManager` 接收 applicationContext 或 sendMessage 更新 `WatchExpenseStore`
